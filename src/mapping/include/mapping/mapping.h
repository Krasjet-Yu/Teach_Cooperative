...
struct RingBuffer {
 public:
  ...
  // search address through idx, when idx < 0, then return (x % N) + N, nor return x
...
struct OccGridMap {
  ...
  */
  ...
  // whether in map or not according idx
 ...
 public:
  ...
  inline const bool isProbOccupied(const Eigen::Vector3i& id) const {
    bool occ_cur = pro.atId(id) > p_occ;
    return isInMap(id) && occ_cur;
  }
